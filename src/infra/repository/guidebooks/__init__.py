...
class GuideBooksRepository:
    cache: dict[
        VERSIONS_ENUM,
        dict[LANGS_GLOBAL_ENUM | LANGS_CHINA_ENUM, dict[str, GuideBook]],
    ] = {}
    ...
    async def load_data(
        ...
        DATA_PATH = Path("./src/infra/database", version, lang, "guidebook.json")
        ...
        for key_id, value_dict in DATA.items():
            ...
            if value_dict.get("id", None):
                self.cache[version][lang].update(
                    {key_id.lower(): GuideBook(**value_dict)}
                )
            else:
                self.cache[version][lang].update(
