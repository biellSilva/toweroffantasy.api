...
class Banner(BaseModel):
    simulacrumId: Annotated[str, BeforeValidator(lambda x: str(x).lower())] | None = (
        Field(
            default=None, validation_alias=AliasChoices("imitation_id", "simulacrumId")
        )
    )
    weaponId: Annotated[str, BeforeValidator(lambda x: str(x).lower())] | None = Field(
        default=None, validation_alias=AliasChoices("weapon_id", "weaponId")
    )
    matrixId: Annotated[str, BeforeValidator(lambda x: str(x).lower())] | None = Field(
