...
class WeaponRepo(ModelRepository[EntityBase, Weapon]):
    ...
    def __init__(self) -> None:
        ...
        self.META_GB: dict[str, dict[str, list[Any]]] = json.loads(
            Path("api/infra/database/global/meta.json").read_bytes()
        )
        self.DESC_VALUES: dict[str, list[float]] = json.loads(
            Path("api/infra/database/global/weaponskillnumbers.json").read_bytes()
        )

        self.__weapon_mats: dict[str, list[list[dict[str, str | int | None]]]] = (
            json.loads(
        ...
        for key_id, value_dict in DATA.items():
                ...
                if description := advancements[0].get("description", None):
                    if "*:" in description:
                        ...
                        if value_dict["id"].lower() == "fan_superpower":
                            weaponEffects.append(
                                {
                                    "title": "Weapon's Master",
            ...
            if value_dict.get("id", None):
                ...
                self.cache[version][lang].update(
                    {key_id.lower(): Weapon(**value_dict, id=key_id)}
                )

        self.cache[version][lang] = {
            i.id: i
            for i in list(
                sorted(list(self.cache[version][lang].values()), key=sort_weapons)
            )
        }
