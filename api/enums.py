...
__cn_simulacra: list[str] = [str(file).lower() for file in loads(Path('api/infra/database/china/cn/imitations.json').read_bytes())]
SIMULACRA_CN = StrEnum('CN_SIMULACRA', __cn_simulacra)

__cn_weapons: list[str] = [str(file).lower() for file in loads(Path('api/infra/database/china/cn/weapons.json').read_bytes())]
