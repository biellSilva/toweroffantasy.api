...
METADATA = {
    "name": "Weapons",
    "description": (
        "Weapons obtained from simulacra in Tower of Fantasy \t\n" "CONTAINS CN DATA"
    ),
}

INCLUDE = {"id", "name", "assets", "category", "rarity", "element", "limited"}
EXCLUDE = {
    "upgradeMats": True,
    "weaponAdvancements": {"__all__": {"multiplier": True}},
    "weaponAttacks": {"__all__": {"__all__": "values"}},
}


@router.get("/{id}", name="Get weapon", response_model=Weapon)
async def get_weapon(
    id: WEAPONS = Path(description="Weapon ID"),
    ...
    **Return** \n
    ...
    if include:
        return ORJSONResponse(
            [weapon.model_dump(exclude=EXCLUDE) for weapon in weapons]
        )

    else:
        return ORJSONResponse(
            [weapon.model_dump(include=INCLUDE, exclude=EXCLUDE) for weapon in weapons]
        )
